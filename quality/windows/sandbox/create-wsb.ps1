...
.EXAMPLE
    ...
    .\dotfiles.wsb  # Launch Sandbox with minimal scenario

.EXAMPLE
    .\create-wsb.ps1
    .\dotfiles.wsb  # Launch Sandbox for manual testing
#>
...
# Resolve repository root (this script is at quality/windows/sandbox/create-wsb.ps1)
$ScriptDir = $PSScriptRoot
$RepoRoot = (Resolve-Path (Join-Path $ScriptDir "..\..\..\")).Path
$LogsRoot = Join-Path $RepoRoot "quality\windows\logs"
...
# Read template
$TemplatePath = Join-Path $ScriptDir "dotfiles.wsb.template"
$OutputPath = Join-Path $ScriptDir "dotfiles.wsb"
...
$Content = $Content.Replace("__DOTFILES_ROOT__", $RepoRoot)
...
Copy-Item 'C:\host-dotfiles' `$WorkDir -Recurse;
cd `$WorkDir;
...
if ($Scenario) {
    ...
    $InnerCommand = $WinGetInstallCmd + $CopyRepoCmd + "`$env:SCENARIO='$Scenario'; .\quality\windows\sandbox\run-in-sandbox.ps1"
} else {
...
Write-Host "  .\dotfiles.wsb" -ForegroundColor Yellow
