...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            cat <<EOF
Usage: dotfiles migrate [--dry-run] [--profiles]

Migrate the dotfiles state file to schema v3.
...
if [[ ! -f "$STATE_PATH" ]]; then
    ...
    log_info "A fresh v3 state will be created on the next 'dotfiles apply'."
    exit 0
fi
