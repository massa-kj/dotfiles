...
- Linux/WSL: `$XDG_CONFIG_HOME/dotfiles/sources.yaml`
- Linux/WSL fallback: `~/.config/dotfiles/sources.yaml`
- Windows: `%APPDATA%\dotfiles\sources.yaml`

`DOTFILES_SOURCES_FILE` may override the path.
...
sources:
  - id: foo
    type: git
    url: https://github.com/foo/dotfiles-features
    commit: abcdefg
    allow:
      features:
        - node
...
sources:
  - id: tools
    type: git
    url: https://github.com/example/tools-dotfiles
    commit: abcdef0
    allow:
      features: "*"
      backends:
