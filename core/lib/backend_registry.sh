...
backend_registry_load_policy() {
    local policy_file="${1:-${DOTFILES_POLICY_FILE:-}}"
...
resolve_backend_for() {
    ...
    if [[ -z "$_BR_POLICY_DATA" ]] && [[ -n "${DOTFILES_POLICY_FILE:-}" ]]; then
...
_backend_registry_platform_default() {
    ...
    case "${DOTFILES_PLATFORM:-linux}" in
        ...
        windows)
            ...
            ;;
        *)
            log_error "_backend_registry_platform_default: unsupported platform: ${DOTFILES_PLATFORM:-unknown}"
            return 1
            ;;
    esac
}
